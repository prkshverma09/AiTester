...
test.describe('Full student journey', () => {
  ...
  test('student navigates from home, reads the first question, and answers it', async ({ page }) => {
    ...
    // Student sees "Question 1 of 12" — they know how many questions to expect
  ...
  test('student completes the full 12-question test and sees their score', async ({ page }) => {
  ...
  test('student retakes the test after seeing their score', async ({ page }) => {
    ...
    // Student rushes through all 12 questions, picking option-0 or filling subjective each time
    for (let q = 0; q < 12; q++) {
    ...
    // Student is back at the beginning: Question 1 of 12
    await expect(page.getByTestId('question-label')).toContainText('Question 1 of 12')
